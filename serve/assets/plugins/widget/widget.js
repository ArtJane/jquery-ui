(function (factory) {
    ...
    $.Template.prototype = {
        ...
        _compile: function(template){        	
            ...
            return new Function(
                '$item',
                'var $template=this,$widget=$item.widget,$parent=$item.parent,$data=$item.data,$index=$item.index,__=[];' +
                'with($data){__.push("' +                                
                template
                    .replace(/"/g, "'")
                    .replace(/[\r\n\t]/g, '')
        ...
        _boolean: function(value){
            if(value){
                return value.replace(/[^\s\|!&><=]+/g, function(m){
                    return '((typeof ' + m + '!=="undefined"&&' + m + ')||false)';
                });
            }else{
                return 'true';
            }
        ...
        _tmpl: function (namespace, name, compile, data, index, parent, widget) {
            ...
            return parent ? ('##' + item.key) : item.html;
        },

        _parseHtml: function (html, item) {
            var level = 0;
            return html
                .replace(/(?:<(\/)?([a-z1-6]+)([^>]*)>)|([^<>\s]+)/g, function (all, slash, tag, props, text) {
                    if (slash) {
                        level--;
                        return "</" + tag + ">";
                    } else if (tag) {
                        if (level === 0) {
                            props = props ? (" data-tmplkey='" + item.key + "' " + props) : (" data-tmplkey='" + item.key + "'");
                        }
                        level++;
                        if (/^(input|img|br|hr)$/.test(tag)) {
                            level--;
                        }
                        return "<" + tag + props + ">";
                    } else {
                ...
                .replace(/##([0-9]+)/g, function (all, key) {
    ...
    $.Widget.prototype = {
        ...
        _createWidget: function (options, element) {
            ...
            this.template = $.template.compile(this.widgetFullName, this.templates);
        ...
        _on: function (suppressDisabledCheck, element, handlers) {
            ...
            $.each(handlers, function (event, handler) {
                function handlerProxy(event) {
                    ...
                    var tmpl = instance._tmplItem(event.currentTarget);
                    var args = widgetSlice.call( arguments, 0 ).concat([tmpl]);
        ...
        _tmpl: function (name, data) {
            return this.template.tmpl(this.widgetFullName, name, data, null, this);
        },

        _tmplItem: function (elem) {
            return this.template.tmplItem(this.widgetFullName, elem);
        }
    };
