/**
 @ 非原版插件，已修改过，如需调整请联系基础系统部前端小组
 @ 主要修改如下(custom)：
   1.支持 YYYY-M-D 格式
   2.返回时间戳（日期），保存在属性data-中
   3.清空data-属性值
   4.点击现在返回指定时区当前日期时间
   5.日期格式根据用户语言设置内置（开发者无需选择）
   6.日期国际化根据用户语言设置内置（开发者无需选择）
   7.添加新参数（formatType：date time dateTime默认）
   8.输出格式调整，如:由2018-7-21调整为2018/7/21
   9.生成隐藏input来存储时间戳
   10.添加新参数定义隐藏input的name属性值（inputName: ['startTime','stopTime']默认）
   11.重置隐藏input
   12.添加皮肤UED蓝 skinCell:"uedblue"
   13.日期转时间戳
   14.修补只选时间返回undefined
   15.只选时间返回本地日期的时间段时间戳
   16.添加参数isClear: true, 是否显示清空按钮(官网有，原代码没有)
   17.修补区间选择时，倒过来选的时候 日期和时间对不上
   18.添加新参数值（formatType: yearStr yearMonthStr dateStr）返回字符串值
   等等...
 */
;(function(root, factory) {
    ...
    var jet = {}, doc = document, regymdzz = "YYYY|MM|DD|hh|mm|ss|zz", gr = /\-/g,
        ...
        config = {
            ...
            languageEN:{                            
                ...
                titText: "Please select the time",
                clear : "Clear",
                today : "Now",
                yes   : "set",
                close : "Close"
            ...
            isClear: true, //是否显示清空按钮 custom
            formatType: 'dateTime', // date->返回年月日 time->返回时分秒 dateTime->返回日期时间 yearStr->返回年字符串 yearMonthStr->返回年月字符串 dateStr->返回年月日字符串 custom
            inputName: ['startTime', 'stopTime'] // 隐藏input的name属性值 custom
        };
    ...
    //转换格式
    jet.dateTransForm = function (sourceTime,formatType) {
        // 目标时区的毫秒
        var targetTimes = parseInt(sourceTime);
        // 目标时区的日期对象
        ...
        if (/-/.test(sourceTime) || sourceTime.length <= 4) {// 非时间戳
            if (format === 'en-GB') {// 英文调整顺序返回
                return sourceTime.split('-').reverse().join('/')
            } else {// 中文去掉补位0返回
                return sourceTime.replace(/-0|-/g, '/');
            }
        }
        // 返回对应的日期时间格式
        switch (format) {
    ...
    jet.timeTransform = function(sourceTimes, formatType) {
        if (!sourceTimes) {
            return sourceTimes;
        }
        var  sourceTime = new Date(sourceTimes.replace(new RegExp("-","gm"),"/")).getTime();
        //服务器所在时区 北京东八区（-8*60）
        var localTimeZone = 8;//本地时区
        var settimeZone = -(new Date().getTimezoneOffset() / 60);//设置时区
        if ($.getCookie('pref-timezone')) { // 有设置时区
            var arrTime = $.getCookie('pref-timezone').split(':');
            if (parseInt(arrTime[0]) >= 0) {
                settimeZone = parseInt(arrTime[0]) + parseInt(arrTime[1]) / 60;
            } else {
                settimeZone = parseInt(arrTime[0]) + (-parseInt(arrTime[1]) / 60);
            }
        }
        var timelag = (settimeZone - localTimeZone) * 3600000;//时差

        var targetTime = parseInt(sourceTime)-timelag;
        return this.dateTransForm(targetTime,"dateTime");
    };
    ...
    jedfn.initdates = function(){
        // 获取偏好语言设置 custom
        var formatType = this.opts.formatType ? this.opts.formatType: config.formatType;
        if ($.getCookie('pref-format')) {
            if ($.getCookie('pref-format') == 'zh-CN') {// 中文
                switch (formatType){
                    case 'date':
            ...
            if ($.getCookie('pref-lang')) {//有语言设置
                if ($.getCookie('pref-lang') == 'zh-CN') {// 中文
                    switch (formatType){
                        case 'date':
                            this.format = 'YYYY-MM-DD';
                            break;
        ...
        this.opts.format = this.format;
        // console.log(this.opts.format); // hh:mm:ss
        
        // 初始隐藏元素 custom
        if (this.opts.range) {
            var name1 = this.opts.inputName ? this.opts.inputName[0]: config.inputName[0];
            var name2 = this.opts.inputName ? this.opts.inputName[1]: config.inputName[1];
            this.valCell.siblings('input[name="'+name2+'"]').remove();
            this.valCell.siblings('input[name="'+name1+'"]').remove();
            this.valCell.after('<input type="text" style="display: none"  name="'+name2+'">')
            this.valCell.after('<input type="text"   style="display: none"  name="'+name1+'">')
        } else {
            var name1 = this.opts.inputName ? this.opts.inputName[0]: config.inputName[0];
            this.valCell.siblings('input[name="'+name1+'"]').remove();
            this.valCell.after('<input type="hidden"  name="'+name1+'" value="'+jet.timeTransform(this.valCell.context.value)+'">')
            // this.valCell.after('<input type="text"  style="display: none"  name="'+name1+'">' )
        }
        ...
        if (opts.range) { // custom
            if (/-/g.test(opts.range)) {
                opts.range = opts.range.replace(/-/g, '=');
            }
        }
    ...
    jedfn.setValue = function (fnStr,matStr,bool) {
        ...
        var type = jet.isValHtml(elCell) ? "val" : "text",
            vals = that.parseValue(strVal, matStr); // 最终输出的原始字符串值vals

        // 支持格式转换 custom
        var formatType = this.opts.formatType ? this.opts.formatType: config.formatType;
        if (formatType == 'yearStr' || formatType == 'yearMonthStr' || formatType == 'dateStr') {// 返回字符串
            if (that.format == 'DD-MM-YYYY' || that.format == 'MM-YYYY') {
                if (that.opts.range) {
                    var arr = vals.split(that.opts.range);
                    var str1 = arr[0].split('-').reverse().join('-');
                    var str2 = arr[1].split('-').reverse().join('-');
                    var valsStr = [str1, str2];
                } else {
                    var valsStr = vals.split('-').reverse().join('-');
                }
            } else if (that.opts.range && that.format == 'YYYY') {
                var valsStr = vals.split(that.opts.range);
            } else {
                if (that.opts.range) {
                    var valsStr = vals.split(that.opts.range);
                } else {
        ...
        if (/YYYY-MM/.test(that.format)) {
            vals = vals.replace(/-0/g, '-');// 页面显示的字符串
        }
        
        // 返回时间戳（日期） custom
        if (vals && this.format !== 'hh:mm:ss') {
            var regFormat = /hh|mm|ss/g;
            var localTimeZone = 8;//本地时区
            var settimeZone = -(new Date().getTimezoneOffset() / 60);//设置时区
            if ($.getCookie('pref-timezone')) { // 有设置时区
                var arrTime = $.getCookie('pref-timezone').split(':');
            ...
            var timelag = (settimeZone - localTimeZone) * 3600000;//时差
            if (regFormat.test(that.opts.format)) { // 带时分秒
                if (that.opts.range) { // 区间 双日期
                    var valsArr = vals.split(that.opts.range);
                    var sourceTimes0 = ($.timeStampDate(valsArr[0],that.format) * 1000) - timelag;
                    var sourceTimes1 = ($.timeStampDate(valsArr[1],that.format) * 1000) - timelag;
                    elCell.attr('data-start', sourceTimes0);
                    elCell.attr('data-end', sourceTimes1);
                    // 隐藏input存值
                    var name1 = that.opts.inputName ? that.opts.inputName[0]: config.inputName[0];
                    var name2 = that.opts.inputName ? that.opts.inputName[1]: config.inputName[1];
                    // elCell.siblings('input[name="'+name1+'"]').val(sourceTimes0);
                    // elCell.siblings('input[name="'+name2+'"]').val(sourceTimes1);
                    elCell.siblings('input[name="'+name1+'"]').val(jet.dateTransForm(sourceTimes0,"dateTime"));
                    elCell.siblings('input[name="'+name1+'"]').val(jet.dateTransForm(sourceTimes1,"dateTime"));
                } else { // 单日期
                    var sourceTimes = ($.timeStampDate(vals,that.format) * 1000) - timelag;
                    elCell.attr('data-day', sourceTimes);
                    // 隐藏input存值
                    var name1 = that.opts.inputName ? that.opts.inputName[0]: config.inputName[0];
                    // elCell.siblings('input[name="'+name1+'"]').val((sourceTimes));
                    elCell.siblings('input[name="'+name1+'"]').val(jet.dateTransForm(sourceTimes,"dateTime"));
                    // console.log("sourceTimes:"+sourceTimes);
                    // console.log("jet.GetDateTime(sourceTimes):"+jet.dateTransForm(sourceTimes,"dateTime"));
                }
            } else { // 不带时分秒
                if (that.opts.range) { // 区间 双日期
                    if (formatType == 'yearStr' || formatType == 'yearMonthStr' || formatType == 'dateStr') {// 返回字符串
                        elCell.attr('data-start', valsStr[0]);
                        elCell.attr('data-end', valsStr[1]);
                        // 隐藏input存值
                        var name1 = that.opts.inputName ? that.opts.inputName[0]: config.inputName[0];
                        var name2 = that.opts.inputName ? that.opts.inputName[1]: config.inputName[1];
                        elCell.siblings('input[name="'+name1+'"]').val(valsStr[0]);
                        elCell.siblings('input[name="'+name2+'"]').val(valsStr[1]);
                    } else {// 返回时间戳
                        var valsArr = vals.split(that.opts.range);
                        var sourceTimes0 = ($.timeStampDate(valsArr[0],that.format) * 1000) - timelag;
                        var sourceTimes1 = ($.timeStampDate(valsArr[1],that.format) * 1000) + (24 * 3600000 - 1) - timelag;
                        elCell.attr('data-start', sourceTimes0);
                        elCell.attr('data-end', sourceTimes1);
                        // 隐藏input存值
                        var name1 = that.opts.inputName ? that.opts.inputName[0]: config.inputName[0];
                    ...
                    if (formatType == 'yearStr' || formatType == 'yearMonthStr' || formatType == 'dateStr') {// 返回字符串
                        elCell.attr('data-day', valsStr);
                        // 隐藏input存值
                        var name1 = that.opts.inputName ? that.opts.inputName[0]: config.inputName[0];
                        elCell.siblings('input[name="'+name1+'"]').val(valsStr);
                    } else {// 返回时间戳
                        // var sourceTimes = ($.timeStampDate(vals) * 1000) + (24 * 3600000 - 1) + timelag;
                        var sourceTimes = ($.timeStampDate(vals,that.format) * 1000) - timelag;
                        elCell.attr('data-day', sourceTimes);
                        // 隐藏input存值
                        var name1 = that.opts.inputName ? that.opts.inputName[0]: config.inputName[0];
        ...
        // 返回时间戳（时间） custom
        if (vals && this.format === 'hh:mm:ss') {
            var localTimeZone = -(new Date().getTimezoneOffset() / 60);//本地时区
            var settimeZone = -(new Date().getTimezoneOffset() / 60);//设置时区
            if ($.getCookie('pref-timezone')) { // 有设置时区
                var arrTime = $.getCookie('pref-timezone').split(':');
                if (parseInt(arrTime[0]) >= 0) {
            ...
            var localYY = localDate.getFullYear();
            var localMM = localDate.getMonth() + 1;
            localMM = localMM < 10 ? ('0' + localMM) : localMM;
            var localDD = localDate.getDate();
            localDD = localDD < 10 ? ('0' + localDD) : localDD;
            var localYMD = localYY+'/'+localMM+'/'+localDD;

            if (that.opts.range) { // 区间 双日期
                var valsArr = vals.split(that.opts.range);
                var dateTime1 = localYMD+','+valsArr[0];
                var dateTime2 = localYMD+','+valsArr[1];
                // new Date('2017/11/11,12:20:12');
                var sourceTimes0 = new Date(dateTime1).getTime() - timelag;
                var sourceTimes1 = new Date(dateTime2).getTime() - timelag;
                elCell.attr('data-start', sourceTimes0);
                elCell.attr('data-end', sourceTimes1);
                // 隐藏input存值
                var name1 = that.opts.inputName ? that.opts.inputName[0]: config.inputName[0];
                ...
                var dateTime = localYMD+','+vals;
                var sourceTimes = new Date(dateTime).getTime() - timelag;
                elCell.attr('data-day', sourceTimes);
                // 隐藏input存值
                var name1 = that.opts.inputName ? that.opts.inputName[0]: config.inputName[0];
                elCell.siblings('input[name="'+name1+'"]').val(sourceTimes);
    ...
    jedfn.getValue = function (valobj) {
        ...
        if (valobj == undefined && jet.isBool(opts.isShow)){
            ...
            if (isValShow) {
                ...
                if (opts.range) {// custom
                    if (/=/g.test(opts.range)) {
                        var rangeStr = opts.range.replace(/=/g, '-');
                    } else {
                        var rangeStr = opts.range;
                    }
                }
                var isunRange = opts.range != false, initVal = that.getValue(),
                    // spVal = initVal.split(opts.range), reMat = jet.reMatch(that.format);// custom
                    spVal = initVal.split(rangeStr), reMat = jet.reMatch(that.format);// custom
    ...
    jedfn.renderHtml = function(ys, ms, ds, opts,boxcls){
        ...
        // 获取语言设置 custom
        if ($.getCookie('pref-lang')) {//有语言设置
            if ($.getCookie('pref-lang') == 'zh-CN') {
                lang = config.languageCN// 中文
            } else {
                lang = config.languageEN// 英文
            }
        } else {//没语言设置
            if ($.getCookie('lang')) {//有国际化设置
                if ($.getCookie('lang') == 'zh') {
                    lang = config.languageCN
                } else {
                    lang = config.languageEN
                }
            } else {//浏览器设置
                var userLanguage = (navigator.browserLanguage || navigator.languages[0]).toLowerCase();
                if (userLanguage.indexOf('zh')!=-1) {
        ...
        var isclear = that.opts.isClear != undefined ? that.opts.isClear: config.isClear;
        if (isclear) {
            // var btnStr = '<span class="clear">'+clearTxt+'</span><span class="today">'+lang.today+'</span><span class="setok">'+lang.yes+'</span>';
            var btnStr = '<span class="clear">'+clearTxt+'</span><span class="setok">'+lang.yes+'</span>';
        } else {
            var btnStr = '<span class="today">'+lang.today+'</span><span class="setok">'+lang.yes+'</span>';
        }
        var timeDiv = $("<div/>",{"class":"timecon"}).append(timeStr());
        var btnsDiv = $("<div/>",{"class":"btnscon"}).append(btnStr);
    ...
    jedfn.createYMHtml = function(ys, ms, opts){
        ...
        if (lang.name == 'cn') {
            ...
            ymText = "<span class='ymbtn'>"+monthArr[month-1]+" "+year+"</span>";
        }
    ...
    jedfn.clickBtn = function (opts,boxCell) {
        ...
        boxCell.on("click",".clear", function(ev) {
            ...
            // 清空data-属性值 custom
            if (that.opts.range) { // 区间 双日期
                elemCell.attr('data-start', '');
                elemCell.attr('data-end', '');
            } else { // 单日期
                elemCell.attr('data-day', '');
            }
            // 清空隐藏input
            var name1 = that.opts.inputName ? that.opts.inputName[0]: config.inputName[0];
            // var name2 = that.opts.inputName ? that.opts.inputName[1]: config.inputName[1];
        ...
        boxCell.on("click",".today", function() {
            ...
            if ($.getCookie('pref-timezone')) { // 有设置时区
                ...
                var timelag = (settimeZone - localTimeZone) * 3600000;//时差
                var loacalTimes = new Date().getTime() + timelag;
                var xDate = new Date(loacalTimes),
                    objVal = {
        ...
        boxCell.on("click",".setok", function(ev) {
            ...
            if(opts.range == false){
                ...
                if (jet.mlen(that.format) == 7){
                    ...
                    // 区间选择时，倒过来选的时候 日期和时间对不上 custom
                    if (!that.areaVal[1]) {
                        that.areaVal[1] = that.areaVal[0];
                    }
                    var val1 = parseInt(that.areaVal[0].replace(/-/g, ""));
                    var val2 = parseInt(that.areaVal[1].replace(/-/g, ""));
                    if (val1 > val2) {
                        that.areaVal.reverse();
                    }
                ...
                // 修补只选时间返回undefined
                $.each(newarea, function (n, val) {// custom
                    newarea[n] = '1'+val;
                });
                $.each(newobj, function (key, val) {// custom
                    newobj['1'+key] = val;
                });
                // console.log(newarea);
                // console.log(newobj);
            ...
            if (opts.range == false){
                ...
                if (/=/g.test(opts.range)) {// custom
                    var rangeStr = opts.range.replace(/=/g, '-');
                } else {
                    var rangeStr = opts.range;
                }
                $.each(new Array(2),function (i,v) {
                    // var tmpval = {}, spra = jet.reMatch(rans.split(opts.range)[i]);// custom
                    var tmpval = {}, spra = jet.reMatch(rans.split(rangeStr)[i]);// custom
                    $.each(jet.reMatch(that.format),function (r,val) {
                        tmpval[val]=spra[r];
                    });
                    objVal.push(tmpval);
        ...
        if(/^[1-9]*[1-9][0-9]*$/.test(date) && dateTest){
            ...
            if (/DD-MM-YYYY/.test(format)) {
                var arrs = jet.reMatch(date),
                    newdate = new Date(arrs[2],parseInt(arrs[1])-1,arrs[0],arrs[3]||0,arrs[4]||0,arrs[5]||0),
