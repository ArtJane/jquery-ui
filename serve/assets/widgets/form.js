define(["jquery", "@widgets/base" /* , "validate", "chosen", "daterangepicker", "jquerytinymce", "jeDate" */ ], function($) {
    ...
    $.widget("ui.form", $.ui.base, {
        ...
        templates: {
            ...
            oItem: /* html */ `
				{{if items}}					
                    {{if row}}
                        <div class="form-group row">
                            <label class="{{labelCol}} col-form-label">{{label}}</label>
                            <div class="{{valueCol}}">
                                <div class="row">
                                    {{tmpl(items) "aItem"}}
                                </div>
        ...
        _handleItems(next) {
            ...
            $.each(this.options.items, (i, item) => {
                if ($.isArray(item)) {
                    this._handleItemA(item);
                } else {
                    this._handleItemO(item, i, this.options.items);
                }
            });
            if (!this.optDefs.length) {
                next.call(this);
            } else {
                $.when.apply($, this.optDefs).done(() => {
                    next.call(this);
                });
            }
        },
        ...
        _handleItemA: function(items) {
            $.each(items, (i, item) => {
                item.col = item.col || "col";
                this._handleItemO(item, i, items);
            });
        },
        ...
        _handleItemO: function(item, i, parent) {
            ...
            if (item.row) {
                ...
                item.valueCol = [];
                $.each(item.row, (i, col) => {
                    item.labelCol.push(col.split("/")[0]);
                    item.valueCol.push(col.split("/")[1]);
                });
                item.labelCol = item.labelCol.join(" ");
                item.valueCol = item.valueCol.join(" ");
            }
        ...
        _clickButton: function(event, { data }) {
            if (data.type === "submit") {
                this.validate((validate, formData) => {
                    if (validate && $.isFunction(data.click)) {
                        data.click.call(this.element[0], event, formData);
                    }
                });
            } else {
