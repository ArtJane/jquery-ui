define(['jquery', 'base', 'notice', 'listpage'], function($){
    ...
    $.widget('ui.cards', $.ui.base, {
            ...
            main: '\
				<div class="col-md-12">\
					...
					<div class="row">\
						...
						{{/if}}\
						<div class="col-md-2">\
							<button type="button" class="btn btn-primary btn-block selectcoupon" {{if scene==="detail"}}disabled{{/if}}>选择卡项</button>\
						</div>\
					</div>\
					<div class="list"></div>\
				</div>'
        ...
        _init: function(){
			...
			this.list = this.element.find('.list').css({"padding-left": 0, "padding-right": 0});
		...
		_clickSelectcoupon: function(event){
			...
			opts.applySite = $("select[name='site']").val();
			opts.applyBiz = $("select[name='biz']").val();

			if(!opts.applySite || !opts.applyBiz){
				return $('<div>').notice({
					type: 'danger',
					message: '请先选择业务范围！',
					time: 2000
		...
		_setDialogContent: function(){
			...
			this.dialogContent = $('<div>').listpage({
				...
				list: {
					url: '/cardCouponLaunch/chooseCard',
					type: 'POST',
				...
				columns: [
					...
					},
					{
						...
						data: 'cardCouponType',
						render: function(data, row){
							return (transCode.cardCouponType && transCode.cardCouponType[data]) || data;
						}
					},
					{
						title: '名称',
		...
		validate: function(callback){
            ...
            $.Deferred(function(dfd){
            	...
            	if(that.options.validate && !list.length){
                    validate = false;
                    $('<div>').notice({
                        type: 'danger',
                        message: '请选择优惠券！',
